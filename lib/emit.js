...
function describe_given({ description, fn, values }) {
	return description || generateDescription_given(fn, values)
}
...
function generateDescription_given(fn, values) {
	if (!fn)
		return values.map(inspect).join(', ')
	let description = formatFunction(fn)
	if (values && values.length > 0) {
		const args = formatArguments(fn, values)
